...
namespace IdiomsCppTest
{
    ...
    namespace ClassesTest
    {
        ...
        namespace TaskC
        {
            ...
            // И на присвоение
            TEST(DivisionOperators, ByIntegerAndAssign)
            {
                const struct
                {
                    std::pair<int, int> left;
            ...
            TEST(DivisionOperators, ByRationalAndAssign)
            {
                const struct
                {
                    std::pair<int, int> left;
            ...
            // *** Секция операторов суммы ***

            TEST(OperSum, WithRational)
            {
                const struct
                {
                    std::pair<int, int> left;
                ...
                for (const auto& value : values)
                {
                    ...
                    Rational by_sum = left + right;

                    EXPECT_EQ(by_sum == result, true);
                }


            } // End OperSum, WithRational

            TEST(OperSum, WithInteger)
            {
                const struct
                {
                    std::pair<int, int> left;
                ...
                for (const auto& value : values)
                {
                    ...
                    Rational by_sum = left + value.right;
            ...
            // *** Секция операторов разности ***

            TEST(OperSubs, WithRational)
            {
                const struct
                {
                    std::pair<int, int> left;
            ...
            } // End OperSubs, WithRational

            TEST(OperSubs, WithInteger)
            {
                const struct
                {
                    std::pair<int, int> left;
            ...
            } // End OperSubs, WithInteger

            TEST(OperSubs, WithIntegerAndAssignee)
            {
                const struct
                {
                    std::pair<int, int> left;
            ...
            } // End OperSubs, WithIntegerAndAssignee

            TEST(OperSubs, WithRationalAndAssignee)
            {
                const struct
                {
                    std::pair<int, int> left;
            ...
            } // End OperSubs, WithRationalAndAssignee
        } // End Task C namespace

        // Task D
        // https://new.contest.yandex.ru/contests/42158/problems?id=40119%2F2022_10_30%2FWVfsCBVPIW
        ...
        // перечисляя последовательные ключи узлов. Типичный пример —
        // иерархия папок в файловой системе. Вася уже выбрал способ хранения дерева: (смотри хедер)
        // Не будем обсуждать, насколько это эффективно.
        // Ваша задача — написать реализации функций Has, Insert и Delete 
        // для этого класса. 
        // В примере с папками в файловой системе функция Has должна проверить,
        // существует ли такая папка, функция Insert должна создать папку
        // (возможно, с промежуточными родительскими папками), 
        ...
        namespace TaskD
        {
            ...
            TEST(MethodHas, EmptyObject)
            {
                ...
                {
                    // Для этих тестов не нужно придумывать конструктор объекта, а значит, не нужно углубляться в это
                    EXPECT_FALSE(true);
                }
